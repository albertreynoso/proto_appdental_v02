...
/// Pantalla de formulario para crear y editar empleados de clínica dental
class EmpleadosFormScreen extends StatefulWidget {
  final Map<String, dynamic>? empleadoExistente;

  const EmpleadosFormScreen({super.key, this.empleadoExistente});
...
class _EmpleadosFormScreenState extends State<EmpleadosFormScreen> {
  ...
  void _cargarDatosExistentes() {
    if (widget.empleadoExistente != null) {
      final Map<String, dynamic> empleado = widget.empleadoExistente!;
  ...
  void _submitForm() {
    if (_formKey.currentState!.validate()) {
      ...
      _showConfirmationDialog(empleadoData);
    }
  }

  void _showConfirmationDialog(Map<String, dynamic> empleadoData) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(
          widget.empleadoExistente != null ? 'Actualizar Empleado' : 'Crear Empleado',
        ),
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text('Nombre: ${empleadoData['nombres']} ${empleadoData['apellido_paterno']} ${empleadoData['apellido_materno']}'),
              Text('DNI: ${empleadoData['dni_empleado']}'),
              Text('Edad: ${empleadoData['edad']} años'),
              Text('Género: ${_generoLabels[empleadoData['genero']]}'),
              Text('Tipo: ${_tipoEmpleadoLabels[empleadoData['tipo_empleado_id']]}'),
              Text('Teléfono: ${empleadoData['numero_telefonico']}'),
              Text('Email: ${empleadoData['email']}'),
              Text('Salario: S/. ${empleadoData['salario']}'),
              Text('Dirección: ${empleadoData['direccion']}'),
            ],
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              _guardarEmpleado(empleadoData);
            },
            child: Text(widget.empleadoExistente != null ? 'Actualizar' : 'Crear'),
          ),
        ],
      ),
    );
  ...
  Future<void> _guardarEmpleado(Map<String, dynamic> empleadoData) async {
    try {
      if (widget.empleadoExistente != null) {
        // Modo edición
        await FirebaseFirestore.instance
            .collection(_collectionName)
            .doc(widget.empleadoExistente!['id'])
            .update(empleadoData);
        ...
        await FirebaseFirestore.instance
            .collection(_collectionName)
            .add(empleadoData);
        _mostrarMensajeExito('Empleado creado correctamente');
      }
      ...
      _mostrarMensajeError('Error al guardar el empleado: $error');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.empleadoExistente != null ? 'Editar Empleado' : 'Nuevo Empleado',
        ),
      ),
      ...
      children: [
        ...
        Expanded(
          child: ElevatedButton(
            ...
            child: Text(
              widget.empleadoExistente != null ? 'Actualizar' : 'Crear',
